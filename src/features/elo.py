...
Features produites :
- elo_p1, elo_p2          : rating avant le match
    ...
    def compute(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Args:
            df: DataFrame avec colonnes
                [player1_id, player2_id, winner, competition_id, played_at]
        Returns:
            df enrichi de [elo_p1, elo_p2, elo_diff, elo_win_prob_p1]
        """
        ...
        for _, row in df.iterrows():
            ...
            comp_weight = self.comp_weights.get(str(row.get("competition_id", "")), 1.0)
            ...
            k1 = self._k_factor(p1_id)
            k2 = self._k_factor(p2_id)
